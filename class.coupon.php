...
class coupon {
     ...
     * @throws Exception
     */
    static public function generate($prefixpost) {
        ...
        $prefix         = filter_var($prefixpost);
        $suffix         = (isset($options['suffix']) ? filter_var($options['suffix'], FILTER_SANITIZE_STRING) : 'WAW' );
     ...
     * @return array
     */
    static public function generate_coupons($maxNumberOfCoupons, $prefix) {
        $coupons = [];
        $data = array();
        for ($i = 0; $i < $maxNumberOfCoupons; $i++) {
            $temp = self::generate($prefix);
            $coupons[] = $temp;
            $data[] = array($temp, $prefix);
        }
      
        return $data;
    }

   /* static public function saveto_db1($maxNumberOfCoupons, $prefix) {
        $coupons = [];
        $data = array();
        for ($i = 0; $i < $maxNumberOfCoupons; $i++) {
            $temp = self::generate($prefix);
...
*/
	...
	public function saveto_db($code,$merchant,$tanggal) {
